...
impl<'a> TraversalVisitor for SemanticAnalyzer<'a> {
    fn visit_stmt(&mut self, s: &parser::ast::Statement) {
        match s {
            ...
            parser::ast::Statement::ReturnStmt(r) => self.visit_return(r),
            parser::ast::Statement::Raise(r) => self.visit_raise(r),
            parser::ast::Statement::BreakStmt(b) => self.visit_break(b),
            parser::ast::Statement::ContinueStmt(c) => self.visit_continue(c),
            ...
            parser::ast::Statement::MatchStmt(m) => self.visit_match(m),
